...
void ConfigurationClass::serializePowerLimiterConfig(PowerLimiterConfig const& source, JsonObject& target)
{
    ...
    for (size_t i = 0; i < INV_MAX_COUNT; ++i) {
        ...
        t["voltage_thresholds_enabled"] = s.VoltageThresholdsEnabled;
        t["voltage_start_threshold"] = roundedFloat(s.VoltageStartThreshold);
        t["voltage_stop_threshold"] = roundedFloat(s.VoltageStopThreshold);
        t["voltage_stop_delay_minutes"] = s.VoltageStopDelayMinutes;
...
void ConfigurationClass::deserializePowerLimiterConfig(JsonObject const& source, PowerLimiterConfig& target)
{
    ...
    for (size_t i = 0; i < INV_MAX_COUNT; ++i) {
        ...
        inv.VoltageThresholdsEnabled = s["voltage_thresholds_enabled"] | POWERLIMITER_INVERTER_VOLTAGE_THRESHOLDS_ENABLED;
        inv.VoltageStartThreshold = s["voltage_start_threshold"] | POWERLIMITER_INVERTER_VOLTAGE_START_THRESHOLD;
        inv.VoltageStopThreshold = s["voltage_stop_threshold"] | POWERLIMITER_INVERTER_VOLTAGE_STOP_THRESHOLD;
        inv.VoltageStopDelayMinutes = s["voltage_stop_delay_minutes"] | POWERLIMITER_INVERTER_VOLTAGE_STOP_DELAY_MINUTES;
