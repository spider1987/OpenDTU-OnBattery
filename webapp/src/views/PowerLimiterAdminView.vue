            ...
            <template v-if="isEnabled">
                <template v-for="(inv, idx) in powerLimiterConfigList.inverters" :key="idx">
                    ...
                    >
                        ...
                        <template v-if="inv.power_source == 0">
                            <InputElement
                                :label="$t('powerlimiteradmin.IndividualVoltageThresholds')"
                                :tooltip="$t('powerlimiteradmin.IndividualVoltageThresholdsHint')"
                                v-model="inv.voltage_thresholds_enabled"
                                type="checkbox"
                                wide
                            />

                            <template v-if="inv.voltage_thresholds_enabled">
                                <InputElement
                                    :label="$t('powerlimiteradmin.IndividualVoltageStartThreshold')"
                                    :tooltip="$t('powerlimiteradmin.IndividualVoltageMeasurementHint')"
                                    v-model="inv.voltage_start_threshold"
                                    min="16"
                                    max="66"
                                    postfix="V"
                                    type="number"
                                ...
                                <InputElement
                                    :label="$t('powerlimiteradmin.IndividualVoltageStopThreshold')"
                                    v-model="inv.voltage_stop_threshold"
                                    min="16"
                                    max="66"
                                    postfix="V"
                                    type="number"
                                ...
                                <InputElement
                                    :label="$t('powerlimiteradmin.IndividualVoltageStopDelay')"
                                    :tooltip="$t('powerlimiteradmin.IndividualVoltageStopDelayHint')"
                                    v-model="inv.voltage_stop_delay_minutes"
                                    min="0"
                                    max="1440"
                                    :postfix="$t('powerlimiteradmin.Minutes')"
                                    type="number"
                                    wide
                                />

                                <div class="row" v-if="individualVoltageThresholdWarning(inv)">
                                    <div class="col-sm-4"></div>
                                    <div class="col-sm-8">
                                        <div class="alert alert-warning" role="alert">
                                            {{ $t('powerlimiteradmin.IndividualVoltageThresholdWarning') }}
                                        </div>
                                    </div>
                                </div>
                            </template>
...
export default defineComponent({
    ...
    methods: {
        ...
        individualVoltageThresholdWarning(inv: PowerLimiterInverterConfig) {
            return inv.voltage_start_threshold <= inv.voltage_stop_threshold;
        },
        getMetaData() {
        ...
        tidyUpInverterConfigs(inverters: PowerLimiterInverterConfig[]): PowerLimiterInverterConfig[] {
            ...
            for (const metaInv of metaInverters) {
                ...
                newInv.voltage_thresholds_enabled = false;
                newInv.voltage_start_threshold = 50;
                newInv.voltage_stop_threshold = 49;
                newInv.voltage_stop_delay_minutes = 5;
