...
void PowerLimiterClass::loop()
{
    ...
    for (auto& upInv : _inverters) {
        upInv->updateIndividualVoltageProtection();
    }
...
uint8_t PowerLimiterClass::getPowerLimiterState() const
{
    ...
    return ((_batteryState == BatteryState::DISCHARGE_ALLOWED
                || _batteryState == BatteryState::DISCHARGE_NIGHT
                || hasIndividualVoltageControlledBatteryInverterAllowed()))
        ...
        PowerLimiterClass::inverter_filter_t filter, std::string const& filterExpression)
{
    ...
    for (auto& upInv : _inverters) {
        ...
        if (upInv->isBatteryPowered()) {
            auto individualAllowed = hasIndividualVoltageControlledBatteryInverterAllowed();
            auto globalBatteryBlocked = _batteryState == BatteryState::STOP
                || (individualAllowed && _batteryState == BatteryState::NO_DISCHARGE);
            auto inverterBlocked = upInv->usesIndividualVoltageThresholds()
                ? !upInv->isIndividualVoltageDischargeAllowed()
                : globalBatteryBlocked;
...
bool PowerLimiterClass::hasIndividualVoltageControlledBatteryInverterAllowed() const
{
    for (auto const& upInv : _inverters) {
        if (upInv->isIndividualVoltageDischargeAllowed()) { return true; }
    }
    return false;
}
...
{
    if (((_batteryState == BatteryState::STOP) || (_batteryState == BatteryState::NO_DISCHARGE))
            && !hasIndividualVoltageControlledBatteryInverterAllowed()) { return 0; }
