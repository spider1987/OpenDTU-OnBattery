...
float PowerLimiterInverter::getAverageDcVoltage()
{
    auto pStats = _spInverter->Statistics();
    auto channels = pStats->getChannelsByType(TYPE_DC);
    float sum = 0.0f;
    size_t count = 0;

    for (auto const& channel : channels) {
        auto voltage = pStats->getChannelFieldValue(TYPE_DC, channel, FLD_UDC);
...
bool PowerLimiterInverter::usesIndividualVoltageThresholds() const
{
    return isBatteryPowered() && _config.VoltageThresholdsEnabled;
}

bool PowerLimiterInverter::isIndividualVoltageDischargeAllowed() const
{
    return usesIndividualVoltageThresholds()
        && _individualVoltageState == IndividualVoltageState::Allowed;
}

void PowerLimiterInverter::updateIndividualVoltageProtection()
{
    if (!usesIndividualVoltageThresholds() || !isReachable()) { return; }

    if (_config.VoltageStartThreshold <= _config.VoltageStopThreshold) {
        if (_individualVoltageState != IndividualVoltageState::Blocked) {
            DTU_LOGE("individual start threshold %.2f V must be above stop threshold %.2f V; disabling inverter",
                    _config.VoltageStartThreshold, _config.VoltageStopThreshold);
        }
        _individualVoltageState = IndividualVoltageState::Blocked;
        _oLowVoltageSinceMillis.reset();
        return;
    }

    auto voltage = getAverageDcVoltage();
    if (voltage <= 0.0f) { return; }

    auto now = millis();

    if (_individualVoltageState == IndividualVoltageState::Unknown) {
        _individualVoltageState = isProducing()
            ? IndividualVoltageState::Allowed
            : IndividualVoltageState::Blocked;
    }

    if (voltage >= _config.VoltageStartThreshold) {
        if (_individualVoltageState != IndividualVoltageState::Allowed) {
            DTU_LOGI("individual voltage %.2f V reached start threshold %.2f V; enabling inverter",
                    voltage, _config.VoltageStartThreshold);
        }
        _individualVoltageState = IndividualVoltageState::Allowed;
        _oLowVoltageSinceMillis.reset();
        return;
    ...
    if (voltage > _config.VoltageStopThreshold) {
        if (_oLowVoltageSinceMillis.has_value()) {
            DTU_LOGI("individual voltage recovered to %.2f V; resetting stop delay", voltage);
        }
        _oLowVoltageSinceMillis.reset();
        return;
    }

    if (_individualVoltageState == IndividualVoltageState::Blocked) { return; }

    // An inverter already in standby must reach the start threshold before it
    // may start again. The delay only protects a currently producing inverter
    // against short voltage dips.
    if (!isProducing()) {
        _individualVoltageState = IndividualVoltageState::Blocked;
        _oLowVoltageSinceMillis.reset();
        return;
    ...
    if (!_oLowVoltageSinceMillis.has_value()) {
        _oLowVoltageSinceMillis = now;
        DTU_LOGI("individual undervoltage %.2f V detected; starting %u minute stop delay",
                voltage, static_cast<unsigned>(_config.VoltageStopDelayMinutes));
    }

    auto delayMillis = static_cast<uint32_t>(_config.VoltageStopDelayMinutes) * 60UL * 1000UL;
    if ((now - *_oLowVoltageSinceMillis) < delayMillis) { return; }

    DTU_LOGI("individual undervoltage %.2f V persisted; disabling inverter", voltage);
    _individualVoltageState = IndividualVoltageState::Blocked;
    _oLowVoltageSinceMillis.reset();
}
